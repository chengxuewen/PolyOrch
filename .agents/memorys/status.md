...
| cmake unit suite | `bash tests/run.sh` | pass=37 fail=0 skip=9 (9 e2e driver-gated) offline; **46/46** with `POLYORCH_TEST_E2E=1`, zero skips (2026-09-22, WP5/WP5b: +`t-rust-crossplan` -- TARGET naming/copy tables, cc-rs trio gate, generated --target/hostbuild/order/no-op legs; +`t-rust-linkplan` -- linker-plan tables + explicit/wrapper/default/CXX/sysroot-gate/run-prefix children; +`t-rust-musl` real cross e2e; `t-rust-executables` CARGO_TARGET leg flipped echo->consumed/gated per the WP5 contract) |
...
| rust offline suite | `bash tests/run.sh` | rust cases 22/22 pass + 8 e2e-skip (2026-09-22, WP5/WP5b) |
...
| generator matrix | `bash tests/matrix.sh` | **5/5 cells** (WP5/WP5b re-run: the new cases ride every cell; `t-rust-musl` self-pins Unix Makefiles like the output-dir single legs, so the MC cell needs no new exclusion): {Makefiles,Ninja}x{Debug,Release} + the WP4 Ninja Multi-Config cell (suite runs -C Debug then -C Release, EXCLUDING the four pre-WP4 fixture cases whose artifact contracts are single-config-shaped; the per-CFG IMPORTED_LOCATION + staging proof is t-rust-output-dir's own MC legs, right-reason-grepped), right-reason greps, ~40s wall; fixture cells compile on the system cargo through the WP3 concrete-toolchain resolution; ctest leg carries the WP0 SKIP veto (FAIL_REG) and marked-skip (SKIP_REG) properties; cells export `POLYORCH_TEST_GENERATOR` beside `POLYORCH_TEST_CONFIG` so the fixture-driver cases build on the cell's generator+config (2026-09-22, WP3 re-run) |
