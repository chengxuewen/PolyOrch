...
| `cmake/PolyOrchFindRust.cmake` | rust toolchain layer (D16 split): setup(FROM system\|pixi) + EXECUTABLE injection, WP3 parity surface (rustup proxy tell + enumeration, `PolyOrch_RUST_TOOLCHAIN` knob with `rustup show` host expansion, `polyorch_rust_version_ok` + `PolyOrch_RUST_MIN_VERSION` floor, `PolyOrchRust::{Rustc,Cargo}` imported handles, `_polyorch_rust_derive_target` seam), triple families, artifact naming, native-static-libs probe, cargo command wrapper (PATH + host-env isolation, PIT-14); WP4 pure staging helpers (`_polyorch_rust_copy_plan` incl. gnullvm deps/, `_polyorch_rust_sanitized_out_dir`) |
| `cmake/PolyOrchRustHelpers.cmake` | rust build-graph API (D13/D14): build / import(metadata) / 4 setters / test / run / clean / install(+export stub), mediator + property carrier, FOLDER, collision guard (PIT-13); WP4 MC profile genex (corr:762/772) + deferred finalize: late per-CFG `IMPORTED_LOCATION(_<CFG>)`/`IMPORTED_IMPLIB(_<CFG>)`, output-dir staging copies, corr:2313 property init (D17) |
...
| cmake unit suite | `bash tests/run.sh` | pass=35 fail=0 skip=8 (8 e2e driver-gated) offline; **43/43** with `POLYORCH_TEST_E2E=1`, zero skips (2026-09-22, WP4: +`t-rust-copy-plan` -- staging-plan + output-dir-sanitize tables; +`t-rust-output-dir` -- late-targetprop/cachevar staging, space paths, MC Debug+Release legs. WP3's `t-rust-findrust` rows unchanged) |
