...
| cmake unit suite | `bash tests/run.sh` | pass=33 fail=0 skip=7 (7 e2e driver-gated) offline; **40/40** with `POLYORCH_TEST_E2E=1`, zero skips (2026-09-22, WP2: +`t-rust-executables` offline; five pixi-gated rust e2e legs flipped skip->execute on the system toolchain -- see rust e2e row) |
...
| rust offline suite | `bash tests/run.sh` | rust cases 18/18 pass + 6 e2e-skip (2026-09-22, WP2) |
| rust e2e (system cargo) | `POLYORCH_TEST_E2E=1 bash tests/run.sh` | the five legs rule-wiring / import-ws / link-c / install-e2e / profile-release now EXECUTE on the system toolchain behind `# requires: system-rust` (the driver composes the child PATH with `~/.cargo/bin`; profile-release composes it itself) -- the WP2 skip->pass flip list; `t-rust-e2e` is the single `# requires: pixi-rust` keeper for the FROM pixi route, gated on the pixi TOOL probe and materializing its own env every run (no store left to pre-gate on after rust-basic dropped pixi; R-12 exposure confined to this one case). Timings (warm caches): the six legs cost ~5.1s after vs ~13.6s reconstructed-before from the HEAD tree (install-e2e alone varied 2.1s<->7.8s run to run -- the pixi-solve variance was the headline motivation); full e2e suite wall 5.8s; negative legs still assert against the driver's raw `_configure.log`/`_build.log`; pow exponents stay runtime (2026-09-22, WP2) |
| generator matrix | `bash tests/matrix.sh` | 4/4 cells {Makefiles,Ninja}x{Debug,Release}, right-reason greps, 27s wall; fixture cells compile on the system cargo now (no per-cell pixi solve); ctest leg carries the WP0 SKIP veto (FAIL_REG) and marked-skip (SKIP_REG) properties; cells export `POLYORCH_TEST_GENERATOR` beside `POLYORCH_TEST_CONFIG` so the fixture-driver cases build on the cell's generator+config (2026-09-22, WP2 re-run) |
