...
| `tests/` | `run.sh` driver + CTest registration; 49 `cmake -P` cases; shared marker contract incl. `# requires: <cap>` capability gates (cases/_requires.cmake; `system-rust` probes PATH + `~/.cargo/bin` since WP2, aligning the parent gate with the driver-composed child PATH), contract-SKIP veto and driver tripwires (D15) |
...
| rust offline suite | `bash tests/run.sh` | rust cases 25/25 pass + 8 e2e-skip (2026-09-22, WP6: +knobs/+pkgver/+defaultkinds) |
| rust e2e (system cargo) | `POLYORCH_TEST_E2E=1 bash tests/run.sh` | the six legs rule-wiring / import-ws / link-c / install-e2e / profile-release / output-dir EXECUTE on the system toolchain behind `# requires: system-rust` (the driver composes the child PATH with `~/.cargo/bin`; profile-release composes it itself); since WP3 these children resolve through the rustup PROXY at `~/.cargo/bin` down to the concrete toolchain -- the proxy branch proven under real rustup 1.29.1. since WP5 the musl leg `t-rust-musl` executes the REAL cross route (rustup target x86_64-unknown-linux-musl: static bin + archive asserted by ELF magic + file(1), hostbuild fall-back to the distinct host dir; the static-PIE reality of rust 1.98 musl linking is documented at the case); `t-rust-link-c` additionally runs the WP5b `polyorch_rust_link_libraries` direction (Rust bin <- CMake C staticlib, marker `POLYORCH_LINKLIB_OK 7`). `t-rust-e2e` remains the single `# requires: pixi-rust` keeper for the FROM pixi route (env materialized every run, R-12 exposure confined to it -- it flaked once during WP3 and passed on re-run for 41/41) AND doubles as the live NON-PROXY proof: conda's rustc answers the `RUSTUP_FORCE_ARG0=rustup` tell as itself and that branch requires no rustup. Full e2e suite wall ~6s warm; negative legs assert against the driver's raw `_configure.log`/`_build.log` (2026-09-22, WP3) |
 cells export `POLYORCH_TEST_GENERATOR` beside `POLYORCH_TEST_CONFIG` so the fixture-driver cases build on the cell's generator+config (2026-09-22, WP3 re-run); WP6 re-run 2026-09-22: the three new cases ride every cell -- 48/49 per cell, the sole failure in all five cells being the same R-12 `t-rust-e2e` conda-forge solve-TLS state as the e2e row above (not a cell regression) |
