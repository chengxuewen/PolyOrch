...
| cmake unit suite | `bash tests/run.sh` | pass=34 fail=0 skip=7 (7 e2e driver-gated) offline; **41/41** with `POLYORCH_TEST_E2E=1`, zero skips (2026-09-22, WP3: +`t-rust-findrust` -- version/floor tables, stub proxy/direct worlds, knob FATAL child legs, real child configure for the handles + the derive seam) |
...
- [ ] **Rust deferred register** (post-D14, explicit non-gaps): cross-`--target` routing + hostbuild (naming table is the seam; WP3 shipped `_polyorch_rust_derive_target` as the code seam); multi-config output staging / `IMPORTED_LOCATION_<CFG>` cluster; macOS install_name rewrites at install; local (crate-scoped, `cargo rustc`) rustflags; INHERITABLE feature propagation; rustc floor gate LANDED in WP3 (`PolyOrch_RUST_MIN_VERSION`) -- still deferred: a cargo-side floor, ceiling/RANGE-exclude forms, and the reference's cross-toolchain version auto-scan (ruled deviation in the port ledger); `Rust_LLVM_VERSION` parsing (not yet reconciled -- WP10 surface pass); package-version exposure as a variable; per-crate native-libs variance; ctest-vs-run.sh SKIP asymmetry (G2 finding, drivers left as-is).
